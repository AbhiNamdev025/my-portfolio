export const SKILLS = [
  { name: 'HTML5', group: 'Frontend' },
  { name: 'CSS3', group: 'Frontend' },
  { name: 'JavaScript', group: 'Frontend' },
  { name: 'React', group: 'Frontend' },
  { name: 'Framer Motion', group: 'Frontend' },

  { name: 'Node.js', group: 'Backend' },
  { name: 'Express.js', group: 'Backend' },
  { name: 'REST APIs', group: 'Backend' },

  { name: 'MongoDB', group: 'Database' },
  { name: 'Mongoose', group: 'Database' },

  { name: 'React Native', group: 'Mobile' },
  { name: 'Expo', group: 'Mobile' },

  { name: 'Figma', group: 'Design' },
  { name: 'Canva', group: 'Design' },
  { name: 'CapCut', group: 'Design' },
  { name: 'Video Editing', group: 'Creative' },
  { name: 'Video Recording', group: 'Creative' },
  { name: 'UI/UX Design', group: 'Design' },
  { name: 'Wireframing', group: 'Design' },
  { name: 'Social Media Management', group: 'Marketing' },
  { name: 'Digital Marketing', group: 'Marketing' },
  { name: 'Content Marketing', group: 'Marketing' },
  { name: 'Meta Ads', group: 'Marketing' },

  { name: 'Git', group: 'DevOps' },
  { name: 'GitHub', group: 'DevOps' },
  { name: 'Vercel', group: 'DevOps' },
  { name: 'Postman', group: 'DevOps' },

  { name: 'ChatGPT', group: 'AI Tools' },
  { name: 'Prompt Engineering', group: 'AI Tools' }
];
